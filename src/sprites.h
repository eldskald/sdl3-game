...
// A sprite from the the spritesheet. The pos_x pos_y mean its position on the
// screen in pixels, its coords_x coords_y mean its coordinates in the
// spritesheet in tiles, or cells, so 0 0 means the first tile, 1 0 means the
// one to its right, and so on. Its coords_w coords_h means the size of the
