...
void DEV_start(void) {
    ...
    // SDL_Log("%s, %s, %d, %d", m.keys[7], m.keys[58], *(int*)m.values[7], *(int*)m.values[58]);

    jsondata* val = JSON_parse("{\"chavea\":43,\"chaveb\":82,\"chavec\":17}");
    SDL_Log("%lu, %lu", val->obj.len, val->obj.cap);
    SDL_Log("%f, %f, %f", ((jsondata*)val->obj.values[7])->num, ((jsondata*)val->obj.values[58])->num, ((jsondata*)val->obj.values[45])->num);
    SDL_Log("%s, %s, %s", val->obj.keys[7], val->obj.keys[58], val->obj.keys[45]);
